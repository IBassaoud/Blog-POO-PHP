...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  $requiredFields = ['pseudo', 'email', 'password', 'repeat_password', 'first_name', 'last_name', 'role'];
  ...
  // If no required field is missing
  if (empty($missingFields)) {
    ...
    if ($_POST["password"] === $_POST["repeat_password"]) {
      ...
      $stmt_ifExist = $db->prepare("SELECT COUNT(*) FROM USERS WHERE `pseudo_user` = :pseudo");
      $stmt_ifExist->execute(['pseudo' => $pseudo]);

      if ($stmt_ifExist->fetchColumn() > 0) {
        $_GET['msg'] = urlencode("User with such username already exists.");
      }

      // Check if email already exists
      $stmt_ifExist2 = $db->prepare("SELECT COUNT(*) FROM USERS WHERE `email_user` = :email");
      $stmt_ifExist2->execute(['email' => $email]);

      if ($stmt_ifExist2->fetchColumn() > 0) {
        $_GET['msg'] = urlencode("User with such email already exists.");
      }

      // If neither username nor email already exists
      if ($stmt_ifExist->fetchColumn() == 0 && $stmt_ifExist2->fetchColumn() == 0) {
    ...
    $msg = urlencode("The following fields are required: " . implode(', ', $missingFields));
    $_GET['msg'] = $msg;
  }
}
    ...
    <form action="<?= $_SERVER['PHP_SELF']; ?>" method="POST">
      ...
      <div class="text-center lg:text-center">
        ...
        <p class="text-sm font-semibold mt-2 pt-1 mb-0">
          ...
          <a href="login.php" class="text-green-600 hover:text-green-700 focus:text-green-700 transition duration-200 ease-in-out">Login here
          </a>
        </p>
